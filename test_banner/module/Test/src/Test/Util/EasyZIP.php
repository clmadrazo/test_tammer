...
/* define(FUNCTION_NOT_FOUND,'Error: gzcompress() function is not found');
  define($this->FILE_NOT_FOUND,'Error: file is not found');
...
class EasyZIP {
    ...
    const FUNCTION_NOT_FOUND = 'Error: gzcompress() function is not found';
    const FILE_NOT_FOUND = 'Error: file is not found';
    const DIRECTORY_NOT_FOUND = 'Error: directory is not found';

    function EasyZIP() {
        if (!@function_exists('gzcompress'))
            die(self::FUNCTION_NOT_FOUND);
    }
    ...
    function setLocalFileHeader() {
        ...
        $this->filename_length = pack('v', strlen(preg_replace('%([^/]+)/%', '', $this->filename))); // 2 bytes
    ...
    function getEndCentralDirectory() {
        ...
        $zipfile_comment = "Compressed by SisRemuq Platfom";
